...
```{python}
def plot_mape(predictions_mean_dict, per, plot_num):
    
    fig, axes = plt.subplots(nrows=plot_num+1, ncols=1, figsize=(16,26))
    
    for model,mean_df in predictions_mean_dict.items():
        all_df = mean_df[per]['All_loc']
        if (all_df is not None):
            all_df[per] = all_df[per].astype(str)
            split_size = len(all_df)//plot_num
            for i in range(0,len(all_df), split_size):
                all_df[i:i+split_size].plot(x=per,
                                            y='mape',
                                            kind='line',
                                            marker='.',
                                            ax = axes[i//split_size], 
                                            label=model)
...
```{python}
plot_mape(predictions_mean_error_dict, 'Location',5)
```

```{python}
plot_mape(predictions_mean_error_dict, 'day_of_week',2)
```

```{python}
plot_mape(predictions_mean_error_dict, 'Date', 5)
```
...
def plot_mae(predictions_mean_dict, per):
    ...
    fig, axes = plt.subplots(nrows=3, ncols=1, figsize=(16,26))
    ...
    for model,mean_df in predictions_mean_dict.items():
        ...
        if (important_df is not None) and (other_df is not None) and (all_df is not None):
            ...
            other_df[split_size:].plot(  
                                    ...
                                    label='demand_mean')
            break
        fig.text(0.5, 0.04, 'MAE error of models', ha='center', va='center')  
```

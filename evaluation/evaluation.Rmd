...
def calculate_accuracy_mape(actual_data, predicted_data, fix_inf = 10):
    
    losses_df = dataset[['Location','Date']].copy()
    losses_df['actual_demand'] = dataset.Demand
...
```{python}
loss_per_location, loss_per_date = calculate_accuracy_mape(dataset, predicted_data)
...
def prepare_report(loss: pd.DataFrame, plot_label):
    ...
    report_dict = {
            ...
            'best_accuracy_location': [loss['error'].idxmin()],
            'worst_accuracy': max_threshhold,
            'worst_accuracy_location': [loss['error'].idxmax()],

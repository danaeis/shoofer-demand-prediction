...
def calculate_loss_mape(actual_data, predicted_data, fix_inf = 1):
    ...
    loss_per_dayofweek = np.sqrt(losses_df.groupby('day_of_week').agg({'error':'mean'}))
...
```{python}
rmse_loss_per_location, rmse_loss_per_date, loss_per_dayofweek = calculate_loss_rmse(dataset, predicted_data)
...
```{python}
date_report_df, loc_report_df, dayofweek_report_df = evaluate_model(dataset, predicted_data, 'mape', 100)
```

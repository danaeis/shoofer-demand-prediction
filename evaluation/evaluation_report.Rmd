...
```{python}
def histogram_demand(actual_df, bin):
    n, bins, patches = plt.hist(actual_df.Demand, bins=bin, color = 'magenta', edgecolor = 'black')
    plt.title('histogram of actual demands')
    plt.show()
...
```{python}
demand_histogram_df = histogram_demand(test_df, 30)
```
...
<!-- #region notebookRunGroups={"groupValue": "2"} -->
plot error histogram for high demand locations
<!-- #endregion -->
...
def plot_mape(errors: pd.DataFrame, per):
    ...
    plt.xlabel(str(per))
    title = 'Model Error per ' + str(per) + ' using method '
    plt.title(title)
...
```{python}
def plot_mape_hist(losses: pd.DataFrame, bin):
    
    bin = int((losses.error.max()-losses.error.min())/bin)
    sb.histplot(losses['error'], kde=True, bins=bin, label='histogram for errors', color='green')

#     plt.hist(losses['error'], color = 'blue', edgecolor = 'black',
        #     bins = int((losses.error.max()-losses.error.min())/bin))
...
```{python}
plot_mape_hist(error_per_location, 5)
```

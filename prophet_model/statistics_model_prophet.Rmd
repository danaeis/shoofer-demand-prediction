...
```{python}
loc = 4
train_data = location_labels_df.iloc[:100,[0,loc]]
test_data = location_labels_df.iloc[100:,[0,loc]]
...
predict = model.predict(future)[['yhat','yhat_lower','yhat_upper']]

```

```{python}
y_true = location_labels_df.iloc[:,loc]
y_pred = predict.yhat
mean_absolute_percentage_error(y_true, y_pred)
```

```{python}
plt.plot(location_labels_df.iloc[:,loc], color='r')
plt.plot(predict)
